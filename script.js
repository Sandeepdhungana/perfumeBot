class PerfumeChatbot {
    ...
    async loadMoreResults() {
        ...
        try {
            ...
            console.log('Load more response ok:', response.ok);
            
            if (!response.ok) {
                const errorText = await response.text();
                console.error('Load more server error:', errorText);
                throw new Error(`HTTP error! status: ${response.status}, message: ${errorText}`);
            }
            ...
            console.error('Error loading more results details:', error);
            this.hideTypingIndicator();
    ...
    async showPerfumeDetails(perfumeName) {
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error('Perfume details error:', errorText);
                throw new Error(`HTTP error! status: ${response.status}, message: ${errorText}`);
            }
            
            const details = await response.json();
            console.log('Perfume details:', details);
